...
def criar_postagem(request):
    if request.method == 'POST':
        form = DesenhoForm(request.POST, request.FILES)
        if form.is_valid():
            desenho = form.save(commit=False)
            desenho.usuario = request.user  # Supondo que você tenha um campo de usuário
            desenho.save()
